...
export default function VoiceScamDetector() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (selectedFile) {
      ...
      const mockFile = new File(["mock"], `${demoTemplate}.mp3`, { type: "audio/mp3" });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errData.detail || "Audio analysis execution failed.");
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-extrabold text-zinc-100 tracking-tight">Voice Scam Detector</h2>
        <p className="text-zinc-400 text-sm">
          Upload suspect call recordings or extortive WhatsApp voice notes. The system filters static using Silero VAD, transcribes utilizing Whisper, and parses threat indicators.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 rounded-xl border border-zinc-850">
            <form onSubmit={handleUpload} className="space-y-6">
              ...
              <div className="border-2 border-dashed border-zinc-850 hover:border-zinc-700 rounded-xl p-8 text-center transition-all cursor-pointer relative bg-zinc-950/20 group">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-zinc-300">
                    {selectedFile ? selectedFile.name : "Select or drag audio recording"}
                  </span>
                  <span className="text-xs text-zinc-500">Supports WAV, MP3, M4A, OGG up to 15MB</span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center">
                <span className="text-[10px] text-zinc-500 font-semibold uppercase">Powered by Agent 2 (Voice Analyzer)</span>
                <button
                  type="submit"
                ...
                >
                  {loading ? <Loader2 className="h-4.5 w-4.5 animate-spin" /> : "Run Speech Diagnostic"}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="space-y-4">
          <div className="glass-card p-4 rounded-xl border border-zinc-850">
            <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-3">Demo Voice Templates</h4>
            <div className="flex flex-col gap-2">
              {demoItems.map((item) => (
                <button
                  key={item.key}
      ...
      {result && (
        <div className={`glass-card p-6 rounded-xl border ${getRiskBorder(result.risk_score)} space-y-6 animate-fadeIn`}>
          ...
          {result.evidence && result.evidence.transcript && (
            <div className="space-y-2 bg-zinc-900/40 border border-zinc-850 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                  <Volume2 className="h-4 w-4 text-emerald-400 animate-pulse" /> Whisper AI Audio Transcript
                </h4>
                <span className="text-[9px] bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 uppercase tracking-wider font-semibold">
                  Silero VAD: Voice Detected
                </span>
              </div>
              <p className="text-xs text-zinc-300 font-serif italic leading-relaxed select-all">
